...
function LoadDeploymentConfig() {
    ...
    if (Test-Path $configPath) {
        ...
        $global:parameterFileMapping = @{}
        if ($null -ne $key) {
            $global:parameterFileMapping = $parameterFileMappings[$key] ?? @{}
        }
        $global:prioritizedContentFiles = $deployment_config.prioritizedcontentfiles ?? @()
        $excludeList = $global:parameterFileMapping.Values + $global:prioritizedContentFiles + ($deployment_config.excludecontentfiles ?? @())
