...
function LoadDeploymentConfig() {
    ...
    $global:exludeContentFiles = @{}
    if (Test-Path $configPath) {
        $deployment_config = Get-Content $configPath | Out-String | ConvertFrom-Json -AsHashtable
        $parameterFileMappings = $deployment_config.parameterfilemappings ?? @{}
        $excludeList = $deployment_config.excludecontentfiles ?? @()
        $global:parameterFileMapping = $parameterFileMappings[$workspaceId] ?? @{}
        $global:prioritizedContentFiles = $deployment_config.prioritizedcontentfiles ?? @()
        $global:exludeContentFiles = $global:parameterFileMapping.Values + $global:prioritizedContentFiles + $excludeList
    }
}
...
function GetParameterFile($path) {
    if ($global:parameterFileMapping.Contains($path.Replace("\", "/"))) {
        $mappedParameterFile = $global:parameterFileMapping[$path.Replace("\", "/")].Replace("/", "\")
...
function DeployOneTemplate($path, $fullDeploymentFlag, $remoteShaTable, $totalFiles, $totalFailed){
...
function Deployment($fullDeploymentFlag, $remoteShaTable, $tree) {
    ...
    if (Test-Path -Path $Directory) 
    {
        ...
        ForEach-Object {
            $path = $_.Replace("/", "\")
            DeployOneTemplate $path $fullDeploymentFlag $remoteShaTable $totalFiles $totalFailed
        }
