...
function Deployment($fullDeploymentFlag, $remoteShaTable, $tree) {
    ...
    if (Test-Path -Path $Directory) 
    {
        $totalFiles = 0;
        $totalFailed = 0;
        $priorityList = ($global:prioritizedContentFiles | ForEach-Object  { $_.Replace("/", "\") })
        $scannedList = (Get-ChildItem -Path $CurrentDir -Recurse -Filter *.json -exclude *metadata.json, *.parameters*.json |
                        Where-Object { $null -eq ( filterContentFile $_.FullName ) } |
                        Select-Object -Property FullName |
                        ForEach-Object { $_.FullName.Replace($CurrentDir + "\", "") })
        $iterationList = $priorityList + $scannedList
